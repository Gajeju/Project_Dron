...
void calc_balancing_with_angle(
  float roll_target_angle, float pitch_target_angle,
...
void print_rpy_balancing(
  float roll_angle, float pitch_angle,
  float yaw_angle, float roll_balancing,
  float pitch_balancing, float yaw_balancing) {
    Serial.print("#RPY: ");
...
void calc_motor_speed(
  float throttle, float roll_balancing,
  float pitch_balancing, float yaw_balancing,
  float &motorA_speed, float &motorB_speed,
  float &motorC_speed, float &motorD_speed) {
    motorA_speed = (throttle == 0) ? 0 : throttle + yaw_balancing + roll_balancing + pitch_balancing;
    motorB_speed = (throttle == 0) ? 0 : throttle - yaw_balancing - roll_balancing + pitch_balancing;
    motorC_speed = (throttle == 0) ? 0 : throttle + yaw_balancing - roll_balancing - pitch_balancing;
    motorD_speed = (throttle == 0) ? 0 : throttle - yaw_balancing + roll_balancing - pitch_balancing;
...
void calc_balancing_with_angle_and_gyro(
  float roll_target_angle, float pitch_target_angle, float yaw_target_angle,
